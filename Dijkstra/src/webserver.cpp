...
int main()
{
    ...
    svr.Get("/api/kortste_pad/(\\d+)/(\\d+)", [&g, &geoid_naar_knoopnr](const httplib::Request &req, httplib::Response &res)
            {
                ...
                else
                {
                    ...
                    res.set_content(graaf_kortste_pad_json(g, kortstePad(g,van_knoopnr, naar_knoopnr)), "application/json");
                } });

    svr.Get("/api/kortste_pad_bidirectioneel/(\\d+)/(\\d+)", [&g, &geoid_naar_knoopnr](const httplib::Request &req, httplib::Response &res)
            {
                int van_geoid = stoi(req.matches[1]);
                int naar_geoid = stoi(req.matches[2]);
                if (!geoid_naar_knoopnr.contains(van_geoid) || !geoid_naar_knoopnr.contains(naar_geoid))
                ...
                else
                {
                    int van_knoopnr = geoid_naar_knoopnr.at(van_geoid); // .at want operator[] is niet const
                    int naar_knoopnr = geoid_naar_knoopnr.at(naar_geoid); // .at want operator[] is niet const
                    res.set_content(graaf_kortste_pad_json(g, kortstePadBidirectioneel(g,van_knoopnr, naar_knoopnr)), "application/json");
                } });

    svr.Get("/api/permanente_knopen", [&g, &geoid_naar_knoopnr](const httplib::Request &req, httplib::Response &res)
