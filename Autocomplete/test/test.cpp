...
bool CheckWord(char* filename,const char* search)
{
    std::ifstream file("woordenlijst.txt");
    char aWord[100];
    bool x = false;
    while (file.good()) {
        
        file>>aWord;
        if (file.good() && strcmp(aWord, search) == 0) {
            x = true;
        }
    }
    if(!x){
...
TEST_CASE("Woordenboek") {
    ...
    //for(auto a : auto_complete){
    //    CheckWord("woordenlijst.txt", a.c_str());
    //}
